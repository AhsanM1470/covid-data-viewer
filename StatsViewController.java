...
/**
 * Defines multiple panes, that can be navigated between, for displaying
 * different statistics, and updates the text labels for the statistics panel
...
public class StatsViewController extends ViewController {
    ...
    protected void initialize() {
        ...
        // Initialises list of statistics panes to be shown
    ...
    /**
     * Creates a FadeTransition to animate the opacity of each statistic from the
     * start value to the end value over the specified duration.
     */
    ...
    /**
     * Processes the data within a given date range and updates the labels
     * accordingly.
     * 
    ...
    /**
     * Hides the currently displayed pane and shows the next pane with a fade
     * transition.
     * 
    ...
    /**
     * Hides the currently displayed pane and shows the previous pane with a fade
     * transition.
     * 
     * @param event The button clicked event that triggered this method
     */
    ...
    /**
     * Updates the text labels for the statistics panel based on the currently
     * selected date range.
     */
    private void refreshLabels() {
        if (!dataset.isDateRangeValid(fromDate, toDate)) {
            ...
            highestDeathDateLabel.setText(
                    dataset.getMostRecentDataWithFilter(dataInDateRange, CovidData::getTotalDeaths).get(0).getDate()
                            + "");
        }
    }

    /**
     * Calculates the total number of deaths for the most recent data within the
     * date range.
     * 
    ...
    private int getTotalNumberOfDeaths() {
        ...
        // gets the most recent record of every borough that is non-null in the
        // total_deaths column
        ArrayList<CovidData> mostRecentDataWithTotalDeaths = dataset.getMostRecentDataWithFilter(dataInDateRange,
    ...
    /**
     * Returns the average total number of Covid cases for the most recent data
     * within the date range.
     *
     * @return the average of total cases in all boroughs within the date range (to
     *         2 d.p.)
     */
    private double getAverageTotalCases() {
        // gets the most recent record of every borough that is non-null in the
        // total_cases column
        ArrayList<CovidData> mostRecentDataWithTotalCases = dataset.getMostRecentDataWithFilter(dataInDateRange,
                CovidData::getTotalCases);
    ...
    /**
     * Returns the average retail and recreation mobility for the Covid Data within
     * the date range.
     * 
     * @return the average RRGMR for the Covid Data within the date range (to 2
     *         d.p.)
     */
    ...
    /**
     * Returns the average grocery and pharmacy mobility for the Covid Data within
     * the date range.
     * 
     * @return the average GPGMR for the Covid Data within the date range (to 2
     *         d.p.)
     */
